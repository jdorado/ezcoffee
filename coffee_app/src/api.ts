...
export type Coffee={id:string;name:string;roast_date:string;notes:string;tag_color?:string;archived?:boolean;revision:number}
export type Shot={id?:string;coffee_id:string;revision:number;date:string;recorded_at?:string;taste_balance?:TasteBalance;choked?:boolean;rating?:number|null;dose:number|null;grind:string;paper:string;temp:string;stop_yield_g?:number|null;yield_g:number|null;target_yield_g?:number|null;target_yield_max_g?:number|null;outcome?:'unrated'|'good'|'adjust'|'bad'|'choked';locked?:boolean;seconds:number|null;first_drip:number|null;pressure:string;basket:string;puck_screen:string;status:string;taste:string;source:string}
