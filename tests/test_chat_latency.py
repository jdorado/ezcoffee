...
class ChatLatency(unittest.IsolatedAsyncioTestCase):
 async def test_auth_only_does_not_load_logbook(self):
  ...
  with patch('src.main.AI_ENABLED',True), patch('src.main.AI_BACKEND','codex'), patch('src.main.authorize',AsyncMock(return_value={})), patch('src.main.state',AsyncMock(side_effect=AssertionError('unneeded read'))) as state:
 ...
 async def test_auth_only_rejects_invalid_capability(self):
  from src.main import agent_context
  with patch('src.main.AI_ENABLED',True), patch('src.main.AI_BACKEND','codex'), patch('src.main.authorize',AsyncMock(side_effect=HTTPException(403))), patch('src.main.state',AsyncMock()) as state:
